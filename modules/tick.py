...
def get_tick_data():
    ...
    for influx_user in influx_read_users:
        ...
        queries['alert_query'] = 'SELECT LAST("crit_duration") AS "crit_duration_before_alerting", LAST("warn_duration") AS "warn_duration_before_alerting", LAST("emitted") AS "deadman_status" FROM "{}"."autogen"."kapacitor_alerts" WHERE time > now() - 28d GROUP BY "metric"'
        list_of_queries = []
