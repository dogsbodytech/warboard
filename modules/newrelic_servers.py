...
def store_newrelic_servers_results():
    ...
    for account in newrelic_data:
        if newrelic_data[account] != None: # Check we actually got some data for a check
            set_data('newrelic_servers_'+account, newrelic_data[account]) # Store it in redis
        else:
            failed_newrelic +=1
            log_messages('Could not get newrelic servers data for '+account, 'error')
        total_accounts+=1
    set_data('total_newrelic_servers_accounts', total_accounts)
    set_data('failed_newrelic_servers', failed_newrelic)

def get_newrelic_servers_results():
    all_results = []
    newrelic_results = {}
    for account in newrelic_keys:
        result_json = json.loads(get_data('newrelic_servers_'+account)) # Pull the NR data from redis load it as json and append to a list
        all_results.append(result_json['servers'])
    newrelic_results['total_newrelic_accounts'] = int(get_data('total_newrelic_servers_accounts'))
    newrelic_results['failed_newrelic'] = int(get_data('failed_newrelic_servers'))
