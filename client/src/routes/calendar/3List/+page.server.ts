...
export async function GET(reqe: RequestEvent) {
	...
	if (newList.find((el) => code == el)) {
		...
		let accountToken = (JSON.parse(listText || '{}') as any).list.find(
		...
		if (accountToken?.token) {
			try {
				...
				let calendarList = await calendar.calendarList.list({ auth: oAuth2Client });

				return {
					body: { calendarList: calendarList.data.items, code, gid }
				};
			} catch (error) {
				return {
					body: { error }
